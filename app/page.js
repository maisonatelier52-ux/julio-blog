    ...
    <main className="relative h-screen w-screen overflow-hidden bg-black text-white">
      ...
      <header className="absolute top-0 left-0 z-20 w-full">
        <div className="mx-auto flex max-w-7xl items-center px-15 py-6">
          ...
          <nav className="ml-10 flex flex-1 items-center justify-between text-sm tracking-widest">
            ...
            {/* White Line on Right of nav */}
            <div className="ml-10 h-px flex-1 bg-white opacity-80" />
          </nav>
        </div>
      </header>
      ...
      <section className="relative z-10 flex h-full items-center">
        <div className="max-w-4xl px-15">
          <p className="font-bold px-1">JHV</p>
      ...
      <footer className="absolute bottom-0 left-0 w-full px-15 pb-15">
        <div className="flex items-center justify-between">
          {/* Line */}
          <div className="h-px flex-1 bg-white opacity-80"></div>
          {/* JHV on right */}
          <span className="ml-4 text-white font-bold tracking-widest">JHV</span>
        </div>
      </footer>
    </main>
  );
